...
#define emit(buf, code) {memcpy(&buf[addr], code, sizeof(code) - 1); addr += sizeof(code) - 1;}
        ...
        emit(rom, "\xf4");                             // [0xfef0] hlt
        emit(rom, "\x90");                             // [0xfef1] nop
#else
        ...
        // Write "HelloWorld!" at physical memory address 0x5000
        emit(rom, "\xbf\x00\x50\x00\x00");             // [0xff08] mov    edi, 0x5000
        emit(rom, "\xb8\x48\x65\x6c\x6c");             // [0xff0d] mov    eax, 'Hell'
        emit(rom, "\x89\x07");                         // [0xff12] mov    [edi], eax
        emit(rom, "\xb8\x6f\x57\x6f\x72");             // [0xff14] mov    eax, 'oWor'
        emit(rom, "\x89\x47\x04");                     // [0xff19] mov    [edi+4], eax
        emit(rom, "\xb8\x6c\x64\x21\x00");             // [0xff1c] mov    eax, 'ld!'
        emit(rom, "\x89\x47\x08");                     // [0xff21] mov    [edi+8], eax
...
#else
        emit(rom, "\x66\xea\xf0\xfe\x0f\x00\x08\x00"); // [0xffe6] jmp    dword 0x8:0x000ffef0
