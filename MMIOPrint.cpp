...
int main() {
    ...
    const size_t length = (strlen(disp_str) % 4 == 0) ? strlen(disp_str) + 1 : strlen(disp_str);
    const size_t row_count = (length + ROW_SIZE - 1) / ROW_SIZE;
...
#define emit(buf, code) {memcpy(&buf[addr], code, sizeof(code) - 1); addr += sizeof(code) - 1;}
        ...
        for (int i = 0; i < row_count; i++) {
            unsigned char tmp[6];
            tmp[0] = '\xbf';
            tmp[1] = (unsigned char)4 * i;
            tmp[2] = '\x00';
            tmp[3] = '\x00';
            tmp[4] = '\xe0';
            emit(ram, tmp);                                // mov    edi, 0xe0000000
            emit(ram, "\x8b\x07");                         // mov    eax, [edi]
            tmp[4] = '\x10';
            emit(ram, tmp);                                // mov    edi, 0x10000000
...
#undef emit
    ...
    vcpu->SetMemoryCallback([](WHV_EMULATOR_MEMORY_ACCESS_INFO* mem) -> HRESULT {
        const size_t length = (strlen(disp_str) % 4 == 0) ? strlen(disp_str) + 1 : strlen(disp_str);
        const size_t row_count = (length + ROW_SIZE - 1) / ROW_SIZE;
        uint32_t *byte_matrix = (uint32_t *)malloc(row_count * sizeof(uint32_t));

        for (size_t i = 0; i < row_count; i++) {
            *(byte_matrix + i) = 0;
            for (size_t j = 0; j < ROW_SIZE; j++) {
                if (i * ROW_SIZE + j < length) {
                    *(byte_matrix + i) += (disp_str[i * ROW_SIZE + j]) << (8 * j);
                }
            }
        }

        for (int i = 0; i < row_count; i++) {
            if (mem->Direction == WHV_IO_IN && mem->GpaAddress == 0xE0000000 + 4*i && mem->AccessSize == 4) {
                *(uint32_t*)&mem->Data = *(byte_matrix + i);
                return S_OK;
            }
        }
        return E_INVALIDARG;
