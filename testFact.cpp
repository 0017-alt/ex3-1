...
#define emit(buf, code) {memcpy(&buf[addr], code, sizeof(code) - 1); addr += sizeof(code) - 1;}
        ...
        // Write n at physical memory address 0x5000
        emit(rom, "\xbf\x00\x50\x00\x00");             // [0xff18] mov    edi, 0x5000
        emit(rom, "\xb8\x04\x00\x00\x00");             // [0xff1d] mov    eax, 0x4
        ...
        emit(rom, "\xb8\x00\x10\x00\x00");             // [0xffb6] mov    eax, 0x1000
        emit(rom, "\x0f\x22\xd8");                     // [0xffbb] mov    cr3, eax
        ...
        emit(ram, "\x89\xc1");                         // [0xff3b] mov    ecx, eax
        emit(ram, "\x89\xc2");                         // [0xff3d] mov    edx, eax
        emit(ram, "\x83\xe9\x01");                     // sub ecx, 1
                                                       // bLoop:
        emit(ram, "\x01\xd0");                         // [0xff4a] add    eax, edx
        emit(ram, "\xe2\xfc");                         // [0xff50] loop   bLoop
